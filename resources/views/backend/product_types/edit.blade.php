...
@section('content')
    <form action="{{ url('product-types', $datas['data']->id) }}" method="post">
        @csrf
        @method('patch')
            ...
            <div class="col-md-12">
                <div class="card">
                    <div class="card-header">{{ $datas['title'] }}
                        <div class="card-tools"><button type="submit" class="btn btn-primary"><i class="fas fa-sync-alt"></i>
                                Update</button> <a href="{{ url('product-types') }}" class="btn btn-default"><i
                                ...
                                <div class="col-6">
                                    <div class="form-group">
                                        ...
                                        <select name="status" class="form-control select2">
                                            ...
                                            <option value="0" {{ $datas['data']->status == 0 ? 'selected' : '' }}>Draft</option>
                                            <option value="1" {{ $datas['data']->status == 1 ? 'selected' : '' }}>Publish</option>
                                        </select>
                                        @error('status')
                                            <small class="error text-red">{{ $message }}</small>
                                        @enderror
