...
@section('content')
    <form action="{{ url('customer', $datas['data']->id) }}" method="post">
        @csrf
        @method('patch')
                ...
                <div class="card">
                    <div class="card-header">{{ $datas['title'] }}
                        <div class="card-tools"><button type="submit" class="btn btn-primary"><i class="fas fa-save"></i>
                                Save</button> <a href="{{ url('customer') }}" class="btn btn-default"><i
                            ...
                            <div class="row mx-3">
                                ...
                                <div class="col-4">
                                    <div class="form-group">
                                        ...
                                        <input type="text" name="phone_number" class="form-control" placeholder="Enter phone number" value="{{ $datas['data']->phone_number ?? old('phone_number') }}">
                                        @error('phone_number')
                                            <small class="error text-red">{{ $message }}</small>
                                        @enderror
                                    </div>
                                ...
                                <div class="col-4">
                                    <div class="form-group">
                                        <label>Email </label>
                                        <input type="email" name="email" class="form-control" placeholder="Enter email" value="{{ $datas['data']->email ?? old('email') }}">
                                        @error('email')
                                            <small class="error text-red">{{ $message }}</small>
                                        @enderror
                                    </div>
